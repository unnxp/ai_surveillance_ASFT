...
def main():
    parser = argparse.ArgumentParser(description="Train EfficientNet-B0 + GRU Action & Crime Classifier (4 Classes, Weighted Loss)")
    ...
    # ตั้งชื่อไฟล์บันทึกแยกต่างหากเพื่อไม่ให้ทับกับโมเดลเดิม
    ...
    print("==================================================")
    print("  Training Action Classifier (4 Classes + Weighted Loss)  ")
    print("==================================================")
    ...
    # 1. Create Datasets & DataLoaders with 32 frames
    print("\nLoading Training & Validation Datasets (4 Classes)...")
    train_dataset = ActionVideoDataset(txt_train, args.dataset_dir, num_frames=args.num_frames, is_training=True)
    valid_dataset = ActionVideoDataset(txt_valid, args.dataset_dir, num_frames=args.num_frames, is_training=False)
    ...
    # 2. Compute Inverse Class Weights for Weighted Loss Function
    print("\nCalculating Inverse Class Weights for Balanced Training...")
    ...
    for cls_id, count in enumerate(class_counts):
        w = total_samples / (len(CLASS_NAMES) * max(1, count))
        class_weights.append(w)
        print(f"  Class {cls_id} ({CLASS_NAMES[cls_id]:<16}): {count:>4} samples | Weight penalty: {w:.4f}")

    weights_tensor = torch.FloatTensor(class_weights).to(device)

    # 3. Create Model, Loss Function (Weighted + Label Smoothing), Optimizer & GradScaler
    print("\nInitializing ActionClassifier (EfficientNet-B0 + GRU)...")
    model = ActionClassifier(num_classes=len(CLASS_NAMES), hidden_size=256, pretrained=True)
    ...
    print("\nStarting Fresh Training Loop (4 Classes + Weighted Loss)...")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, (frames, labels) in enumerate(train_loader, 1):
            frames = frames.to(device) # (Batch, 32, 3, 224, 224)
    ...
    # Save metadata
    meta_info = {
        "model_architecture": "EfficientNet-B0 + GRU (4 Classes Super-Consolidated)",
