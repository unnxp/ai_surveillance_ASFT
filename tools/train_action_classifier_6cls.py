...
from ai.dataset import ActionVideoDataset, CLASS_NAMES_6CLS
from ai.action_classifier import ActionClassifier

def main():
    parser = argparse.ArgumentParser(description="Train / Fine-tune Action Classifier (6 Classes + Weighted Loss)")
    ...
    parser.add_argument("--model-name", type=str, default="action_classifier_6cls", help="Base filename for saving model & metadata")
    ...
    print("==================================================")
    print("  Training / Fine-tuning Action Classifier (6 Classes)  ")
    print("==================================================")
    ...
    # 1. Create Datasets & DataLoaders for 6 classes
    print("\nLoading Training & Validation Datasets (6 Classes)...")
    train_dataset = ActionVideoDataset(txt_train, args.dataset_dir, num_frames=args.num_frames, is_training=True, num_classes_schema=6)
    valid_dataset = ActionVideoDataset(txt_valid, args.dataset_dir, num_frames=args.num_frames, is_training=False, num_classes_schema=6)
    ...
    # 2. Compute Inverse Class Weights for 6 Classes
    print("\nCalculating Inverse Class Weights for 6 Classes...")
    class_counts = [0] * len(CLASS_NAMES_6CLS)
    for _, label in train_dataset.video_list:
        class_counts[label] += 1
        
    total_samples = len(train_dataset.video_list)
    class_weights = []
    print("Class distribution in Train Set (6 Classes):")
    for cls_id, count in enumerate(class_counts):
        w = total_samples / (len(CLASS_NAMES_6CLS) * max(1, count))
        class_weights.append(w)
        print(f"  Class {cls_id} ({CLASS_NAMES_6CLS[cls_id]:<16}): {count:>4} samples | Weight penalty: {w:.4f}")

    weights_tensor = torch.FloatTensor(class_weights).to(device)

    # 3. Create Model Architecture & Optionally Load Base Weights
    print("\nInitializing ActionClassifier (6 Classes)...")
    model = ActionClassifier(num_classes=len(CLASS_NAMES_6CLS), hidden_size=256, pretrained=(args.weights is None))
    ...
    print("\nStarting Training / Fine-tuning Loop (6 Classes)...")
    ...
    # Save metadata
    meta_info = {
        "model_architecture": "EfficientNet-B0 + GRU (6 Classes)",
        "num_classes": len(CLASS_NAMES_6CLS),
        "class_names": CLASS_NAMES_6CLS,
